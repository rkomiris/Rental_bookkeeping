...
@Component({
  selector: 'app-room-booking-configuration-modfiy',
  templateUrl: './room-booking-configuration-modfiy.component.html',
  styleUrls: ['./room-booking-configuration-modfiy.component.css']
})
export class RoomBookingConfigurationModfiyComponent implements OnInit {
    ...
    let viewConfigList = this.roomBookingConfigurationAddService.viewProjectList(localStorage.getItem('roomWorkFlowId')).subscribe(data => {
      ...
      if (resp.succesObject[0].roomWorkFlowSeqVoList != null) {
        ...
        for (let j = 0; j < dynamicRows.length; j++) {
          ...
          this.roomBookingConfigurationAddService.load_selectBox_subLocationData(dynamicRows[j].id).subscribe(
            data => {
              let resp = JSON.parse(data['_body']);
              this.dynamicDeptList[j] = resp.succesObject;
              console.log(this.userDepartmentList[j]);
            },
            error => {
              alert(error);
            }
        ...
        // console.log(resp.succesObject[0].roomWorkFlowSeqVoList[1].roomWorkFlowSlaVo[0].roomWorkFlowSla);
        // this.qtd[1][0] = resp.succesObject[0].roomWorkFlowSeqVoList[1].roomWorkFlowSlaVo[0].roomWorkFlowSla;
        console.log(resp.succesObject[0].roomWorkFlowSeqVoList);
        if (resp.succesObject[0].roomWorkFlowSeqVoList !== undefined) {
          for (let k in resp.succesObject[0].roomWorkFlowSeqVoList) {
            console.log(resp.succesObject[0].roomWorkFlowSeqVoList[k]);
            for (let m in resp.succesObject[0].roomWorkFlowSeqVoList[k].roomWorkFlowSlaVo) {
              console.log(resp.succesObject[0].roomWorkFlowSeqVoList[k].roomWorkFlowSlaVo[m]);
              this.qtd[k][m] = resp.succesObject[0].roomWorkFlowSeqVoList[k].roomWorkFlowSlaVo[m].roomWorkFlowSla;
            }
          }
        }
      }
      console.log(resp.succesObject[0]);
      if(resp.succesObject[0].roomWorkFlowSeqVoList === null){
  ...
  selected_sequenceLocationType(eve, i) {
    this.subLocationNameSeq_data[i] = [];
    this.dynamicUserRoleList[i] = [];
    this.dynamicUserList[i] = [];
    ...
    this.roomBookingConfigurationAddService.load_selectBox_subLocationData(eve).subscribe(
      data => {
        let resp = JSON.parse(data['_body']);
        this.dynamicDeptList[i] = resp.succesObject;
  ...
  onSubmit() {
    if (this.saveForm.valid) {
      ...
      this.roomBookingConfigurationAddService.modifyProjectList(formValue).subscribe(data => {
