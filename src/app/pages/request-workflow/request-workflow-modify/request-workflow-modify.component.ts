...
import { RequestWorkflowModifyService } from './request-workflow-modify.service';
...
@Component({
  selector: 'app-request-workflow-modify',
  templateUrl: './request-workflow-modify.component.html',
  // styleUrls: ['./request-workflow-modify.component.css']
  styleUrls: ['./request-workflow-modify-srmav.component.css']
})
export class RequestWorkflowModifyComponent implements OnInit, OnDestroy {
  ...
  sequanceOne(data) {
    ...
    if (data.value != 0 && data.value != undefined) {
      ...
      // this.saveForm.patchValue({ reqWorkFlowSeqLevelTypeOne: 0 });
    }
    if (data.value == '1') {
      ...
      if (RWF_ModifyList_TableData.requestWorkFlowExecuterVo !== null && RWF_ModifyList_TableData.requestWorkFlowExecuterVo.length > 0) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          this.selected_ExsequencesubLocationType(RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i].executerSublocationId, i,
            ...
            RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i].executerLocationId)

          let row = {};
          row['value'] = RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i];
          this.dynamicExGetUSerRole(RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i].executerDepartmentId, i, row, RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i].executerLocationId);
          this.dynamicExFieldsGetUSer(RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i].executerRoleId, i, row, RWF_ModifyList_TableData.requestWorkFlowExecuterVo[i].executerLocationId);
  ...
  onSubmit() {
    ...
    if (this.saveForm.valid) {
      ...
      if (this.saveForm.value.requestWorkFlowSequenceList.length > 0) {
  ...
  selected_ExsequencesubLocationType(eve, i, Row, locId) {
    if (locId != null && eve != null) {
      this.requestWorkflowAddService.load_ExselectBox_subLocationData({ userLocation: locId, sublocationId: eve }).subscribe(
        data => {
          let resp = JSON.parse(data['_body']);
  ...
  dynamicExGetUSerRole(val, i, Row, locId) {
    /** USER ROLE WITH LOC & SUBLOC & DEPT */
    if (Row.value.executerLocationId != null && Row.value.executerSublocationId != null && val != null) {
  ...
  dynamicExFieldsGetUSer(val, i, row, locId) {
  ...
  customrequestWorkFlowDetailsVoListClick(locationId, subLocationId, refdepartmentId, isActive, currrentRow) {
    this.allFlag = false;
  ...
  selected_ExsequenceLocationType2(eve, i, form) {
  ...
  selected_ExsequencesubLocationType2(eve, i, Row, form) {
