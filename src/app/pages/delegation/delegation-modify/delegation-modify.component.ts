...
import { DelegationModifyService } from './delegation-modify.service';
...
@Component({
  selector: 'app-delegation-modify',
  templateUrl: './delegation-modify.component.html',
  styleUrls: ['./delegation-modify.component.css']
})
export class DelegationModifyComponent implements OnInit {
  ...
  constructor(private componentLoaderService: ComponentLoaderService,
    ...
    private delegationModifyService: DelegationModifyService,private jsonApiService: JsonApiService
  ) { }
        ...
        if (data == true) {
          ...
          let viewData = this.delegationModifyService.viewData(this.delegationId, localStorage.getItem('detailId')).subscribe(
            data => {
              ...
              row.userDelegationDetailsVo[0].delegationRemarks = loadDataList.userDelegationDetailsVo.delegationRemarks;
  ...
  reset() {
    ...
    // this.saveForm.controls['userDelegationDetailsVo'].reset();
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => this.router.navigate(['/delegation/delegation-modify']));
  }

  dateChange(form){
    form.controls.userDelegationDetailsVo.controls[0].controls.userActiveTo.reset();
  }
