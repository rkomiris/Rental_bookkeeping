...
@Component({
  ...
  styleUrls: ['./user-map-srmav.component.css']
})
export class UserMapComponent implements OnInit {
  ...
  onSubmitSearch(val) {
    ...
    for (let i = 0; i < formValue.searchDatas.length; i++) {
      if (formValue.searchDatas[i].__proto__ === Object) {
      }
    }
    for (let i = 0; i < formValue.searchDatas.length; i++) {
    ...
    this.userMapService.search_list(finalSearchData).subscribe(data => {
  ...
  flashNews_list_details() {
    let loadFlashNewsList = this.userMapService.load_flashNewsData().subscribe(data => {
      let flashNewsListGetData = JSON.parse(data['_body']);
      let flashNewsListTableDate = flashNewsListGetData.succesObject;
      this.dataSource = [];
      //if (flashNewsListGetData.succesObject.userMappingVoList !== null) {
        if (flashNewsListTableDate.length > 0){
       // this.dataSource = new MatTableDataSource(flashNewsListGetData.succesObject.userMappingVoList);
      ...
      if(localStorage.getItem('langCode') == 'en'){
        search = [
          { Name: 'User Role Name', Value: 'userRoleName' },
          { Name: 'User Name', Value: 'userName' },
          { Name: 'User Location Name', Value: 'userLocationName' },
          { Name: 'User Department Name', Value: 'userDepartmentName' },
          { Name: 'Reporting To Department Name', Value: 'reportingDepartmentName' },
          { Name: 'Reporting To User Name', Value: 'reportingToUserName' },
          { Name: 'Level', Value: 'levelName' }
        ];
      }
      else if(localStorage.getItem('langCode') == 'jp'){
        search = [          
        { Name: "ロール", Value: 'userRoleName' },
          ...
          { Name: '報告ユーザー部門', Value: 'reportingDepartmentName' },
          { Name: 'ユーザーへの報告', Value: 'reportingToUserName' },
          { Name: 'レベル', Value: 'levelName' }
        ];
      }
      for (let k in search) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.selection.selected.forEach(item => {
            ...
            kk = item;
            let flashNewsType = [];
            flashNewsType.push(kk.userMappingId);
            this.userMapService.deleteProjectList(flashNewsType).subscribe(data => {
    ...
    if (this.selection.selected.length === 1) {
      ...
      dialogRef.afterClosed().subscribe(data => {
        if (localStorage.getItem('isCancelled') === 'No') {
          this.componentLoaderService.display(true);
          this.router.navigate(['/user-map/user-map-modify']);
          let rowId = String(this.selection.selected[0]['userMappingId']);
          localStorage.setItem('userMappingId', rowId);
  ...
  projectView() {
    if (this.selection.selected.length === 1) {
      this.componentLoaderService.display(true);
      this.router.navigate(['/user-map/user-map-view']);
      let rowId = String(this.selection.selected[0]['userMappingId']);
      localStorage.setItem('userMappingId', rowId);
    } else if (this.selection.selected.length > 1) {
      const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
