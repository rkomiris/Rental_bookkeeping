...
@Component({
  ...
  styleUrls: ['./sublocation-srmav.component.css']
})
export class SublocationComponent implements OnInit {
  ...
  onSubmitSearch(val) {
    ...
    this.sublocationService.search_list(finalSearchData).subscribe(data => {
  ...
  sublocation_list_details() {
    let loadSublocationList = this.sublocationService.load_sublocation().subscribe(data => {
      ...
      if (localStorage.getItem('langCode') == 'en') {
        search = [
          { Name: "Code", Value: 'subLocationCode' },
        ...
        search = [
          { Name: "記号", Value: 'subLocationCode' },
          { Name: "場所の名前", Value: 'userLocationName' },
  ...
  removeSelectedRows() {
    if (this.selection.selected.length === 1) {
      ...
      let sublocationId = [this.selection.selected[0].sublocationId];
      ...
      for (let i = 0; i < this.selection.selected.length; i++) {
        sublocationId.push(this.selection.selected[i].sublocationId);
      }
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.sublocationService.deleteProjectList(sublocationId).subscribe(data => {
  ...
  projectModify() {
    if (this.selection.selected.length === 1) {
      ...
      dialogRef.afterClosed().subscribe(data => {
        if (data) {
          this.router.navigate(['/sublocation/sublocation-modify']);
          let rowId = String(this.selection.selected[0]['sublocationId']);
          localStorage.setItem('sublocationId', rowId);
          this.componentLoaderService.display(true);
        }
      });
    } else if (this.selection.selected.length > 1) {
  ...
  projectView() {
    if (this.selection.selected.length === 1) {
      this.componentLoaderService.display(true);
      this.router.navigate(['/sublocation/sublocation-view']);
      let rowId = String(this.selection.selected[0]['sublocationId']);
      localStorage.setItem('sublocationId', rowId);
    } else if (this.selection.selected.length > 1) {
      const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
