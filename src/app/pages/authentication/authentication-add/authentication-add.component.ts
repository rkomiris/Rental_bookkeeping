...
import { AuthenticationAddService } from './authentication-add.service';
...
@Component({
  ...
  styleUrls: ['./authentication-add-srmav.component.css']
})
export class AuthenticationAddComponent implements OnInit {
 ...
 /* onloadSelectboxData(){
  ...
  filterItem(value){
    this.comboList.screenComboList = this.screenList;
    if(!value){
        this.comboList.screenComboList = this.screenList;
    } // when nothing has typed
    this.comboList.screenComboList = Object.assign([], this.comboList.screenComboList).filter(
  ...
  loadAuth_details() {
    ...
    let loadScreenAuthList = this.authenticationServiceService.getAddAuthList().subscribe(
      data => {
        ...
        this.screenList = ListGetData.succesObject.screenComboList;
        this.comboList = ListGetData.succesObject;
        this.userName = ListGetData.succesObject.userName;
        let temp = {
          menuStatus: 200,
      ...
      if (ev.checked === true) {
        let loadScreenAuthList = this.authenticationServiceService.loadSubScreenFields(data, screenId).subscribe(
          data => {
            ...
            for (let i = 0; i < ListGetData.succesObject.length; i++) {
              if (ListGetData.succesObject[i].result === true) {
                this.selectFieldsMAnual(ListGetData.succesObject[i].id, ListGetData.succesObject[i].baseFilter)
              }
            }
          },
          error => {
        ...
        let loadScreenFnList = this.authenticationServiceService.loadScrFn(data, screenId).subscribe(
          data => {
            ...
            for (let i = 0; i < ListGetData.succesObject.length; i++) {
              if (ListGetData.succesObject[i].result === true) {
                this.selectFunctionsMAnual(ListGetData.succesObject[i].id)
              }
            }
          },
          error => {
