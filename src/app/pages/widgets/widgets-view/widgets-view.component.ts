...
@Component({
  selector: 'app-widgets-view',
  templateUrl: './widgets-view.component.html',
  // styleUrls: ['./widgets-view.component.css'],
  styleUrls: ['./widgets-view-srmav.component.css'],
  // changeDetection: ChangeDetectionStrategy.OnPush
})
export class WidgetsViewComponent  implements OnInit, AfterViewInit, OnDestroy {
  saveForm: FormGroup;
  count: any = 1;
  ...
  sequenceType() {
    return this.formBuilder.group({
      ...
      widgetDetailAnnouncementDate: [null],
      widgetDetailValidFrom: [null],
      widgetDetailValidTo: [null],
    });
  }
      ...
      for (let key in element) {
        if (key !== 'widgetDetailPicPath' && key !== 'widgetDetailAttPath') {
          ...
          if (key == 'widgetDetailAnnouncementDate') {
            if (element[key] != null) {
              _temp[key] = moment(element[key]).format('DD/MM/YYYY');
            }
